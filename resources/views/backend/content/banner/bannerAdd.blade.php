...
<script>
	...
	$("#form-banner").submit(function(event) {
		...
		if($.isNumeric(id)){
			var path = ServeUrl+"/banner/"+Instansi+"/update/"+id;
		}else{
			var path = ServeUrl+"/banner/"+Instansi+"/create";
		}
				$.ajax({
							data: data,
							url: path,
							...
							complete: function(response){                
							  if(response.status == 201){
								  ...
								  swal({
										...
										type:'success',
										onClose: function () {
												window.location.replace(BaseUrl+'/admin/bannerAds/');
										}
									}); 
	...
	function loadView(){
		...
		$.ajax({
					data: "",
					url: ServeUrl+"/banner/"+Instansi+"/view/"+id,
                    crossDomain: false,
                    method: 'GET',
                    complete: function(response){ 				
