...
@section('content')
		...
		<div class="content-wrapper">
			...
			<!-- Content area -->
			<div class="content">
			<form id="form-header" class="form-horizontal" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="col-md-7">
									...
									<div class="form-group row">
										...
										<div class="col-lg-8">
										<input type="number" id="order_menu" name="order_menu" class="form-control" placeholder="Order Menu" value=""/>
									...
									<div class="form-group row">
										...
										<div class="form-check form-check-inline">
											<label class="form-check-label">
												<input type="radio" class="form-check-input" name="status" value="Y" checked="">
												Yes
											</label>
										</div>
										<div class="form-check form-check-inline">
											<label class="form-check-label">
												<input type="radio" class="form-check-input" name="status" value="N" checked="">
												No
											</label>
										</div>
										<i class="small"> </i>
...
<script> 
	$("#form-header").submit(function(event) {
		...
		if($.isNumeric(id)){
			var path = ServeUrl+"/headerMenu/"+Instansi+"/update/"+id;
		}else{
			var path = ServeUrl+"/headerMenu/"+Instansi+"/create";
		}
				$.ajax({
							data       : data,
							url        : path,
							...
							complete: function(response){                
							  if(response.status == 201){
								  ...
								  swal({
										...
										type :'success',
										onClose: function () {
												window.location.replace(BaseUrl+'/admin/headerMenu');
										}
									}); 
	...
	function loadView(){
		...
		$.ajax({
					data: "",
					url: ServeUrl+"/headerMenu/"+Instansi+"/view/"+id,
                    method: 'GET',
                    complete: function(response){ 				
	...
	function loadParent(){
		$.ajax({
					data: "",
					url: ServeUrl+"/headerMenu/"+Instansi+"/list_all",
							method: 'GET',
							complete: function(response){ 				
								if(response.status == 200){
							var append = '';
	...
	function loadCreate(){
		$.ajax({
					data: "",
					url: ServeUrl+"/headerMenu/"+Instansi+"/list_create",
                    method: 'GET',
                    complete: function(response){ 				
                        if(response.status == 200){
							var append = '';
							...
							$.each(response.responseJSON.data.kategori, function(k,v){
