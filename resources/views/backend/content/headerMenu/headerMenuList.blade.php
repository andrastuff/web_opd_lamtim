...
<script>
	...
	var table 	= $('.datatable').DataTable( { 
		...
		"ajax": {
            "url": ServeUrl+"/headerMenu/"+Instansi+"/list",//ServeUrl+"api/headerMenu/list",
	...
	function add(){
		window.location.href = "{{url('admin/headerMenu/add/')}}"; 
	}

	$('#datatable-headermenu tbody').on( 'click', '#edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
		window.location.href = BaseUrl+"/admin/headerMenu/update/"+data['id'];
    } );
	...
	function remove(instansi, id){
	          swal({
				...
				$.ajax({
							data: "",
							url : ServeUrl+"/headerMenu/"+Instansi+"/delete/"+id,
							method: 'GET',
							complete: function(response){                
