...
<script>
	...
	var table 	= $('.datatable').DataTable( { 
		...
		"ajax": {
            "url": ServeUrl+"/headline/"+Instansi+"/list",
	...
	$('#datatable-artikel tbody').on( 'click', '#lanjut', function () {
        var data = table.row( $(this).parents('tr') ).data();
        //take one row data from table with id #datatable-artikel when user clicked a button with #lanjut id

		window.location.href = BaseUrl+"api/artikel/lanjut/"+data['id'];
		//then send the id from "data" variabel to route "http://localhost/links/admin/artikel/lanjut/{id}"
    } );
	...
	function loadArtikel(){
		$.ajax({
			data: "",
			url: ServeUrl+"/headline/"+Instansi+"/list_modal",
	        ...
	        complete: function(response){ 				
	            ...
	            //if response status is 200 execute script below
					...
					$.each(response.responseJSON.data, function(k,v){
						append +='<option value=' + v.id + '>' + v.judul_artikel.toUpperCase() + '</option>';
					});
	...
	$("#form-headline").submit(function(event) {
		...
		var path = ServeUrl+"/headline/"+Instansi+"/create";
				$.ajax({
							data: data,
							url: path,
							method: 'GET',
	...
	function remove(instansi, id){
	          swal({
				...
				$.ajax({
							data: "",
							url: ServeUrl+"/headline/"+instansi+"/update/"+id,
							crossDomain: false,
							method: 'GET',
							complete: function(response){                
							  if(response.status == 201){			  
