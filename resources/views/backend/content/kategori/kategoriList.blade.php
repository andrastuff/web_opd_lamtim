...
<script>
	...
	$('#datatable-artikel tbody').on( 'click', '#edit', function () {
		...
		window.location.href = BaseUrl+"/admin/kategori/update/"+data['id'];
    } );

    $('#datatable-artikel tbody').on( 'click', '#delete', function () {
        var data = table.row( $(this).parents('tr') ).data();
		remove(data['id']); 
    } );

	function add(){
		window.location.href = BaseUrl+"/admin/kategori/add/"; 
	}
	...
	function remove(id){
	          swal({
				...
				$.ajax({
							data: "",
							url: ServeUrl+"/kategori/"+Instansi+"/delete/"+id,
							crossDomain: false,
							method: 'GET',
							complete: function(response){                
