...
<!-- Main content -->
			...
			<div class="content">
				...
				<div class="row">
					...
					<div class="text-center"><div class="loader text-center"></div><a id="loadmore" class="btn bg-teal-700 pr-5 pl-5" onclick="loadMore()" data-value=""><strong><i class="fa fa-circle-o-notch"></i> Load More</strong></a></div>   
					</div>
					</div>
...
<script>
	...
	function loadMedia(data){
		$.ajax({
					data: data,
					url: ServeUrl+"/media/"+Instansi+"/list",
                    crossDomain: false,
                    method: 'GET',
	...
	function remove(id){
		...
		swal({
				...
				$.ajax({
							data: "",
							url: ServeUrl+"/media/"+Instansi+"/delete/"+id,
							method: 'GET',
							complete: function(response){                
	...
	$('.file-input-ajax').fileinput({
				browseLabel       : 'Browse',
				uploadUrl         : ServeUrl+"/media/"+Instansi+"/upload", // server upload action
				uploadAsync       : true,
				showUpload        : false,
				maxFileCount      : 3,
				initialPreview    : [],
