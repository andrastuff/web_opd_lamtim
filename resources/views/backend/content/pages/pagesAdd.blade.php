...
<script>
	...
	function loadView(){
		...
		$.ajax({
					data: "",
					url: ServeUrl+"/pages/"+Instansi+"/view/"+id,
                    crossDomain: false,
                    method: 'GET',
	...
	$("#form-pages").submit(function(event) {
		...
		var id = window.location.pathname.split('/').pop();
		if($.isNumeric(id)){
			var path = ServeUrl+"/pages/"+Instansi+"/update/"+id;
		}else{
			var path = ServeUrl+"/pages/"+Instansi+"/create";
		}
				$.ajax({
							data: data,
							url: path,
	...
	function sendFile(file, editor, welEditable) {
            ...
            $.ajax({
                data: data,
                method: "POST",
                url: ServeUrl+"/pages/"+Instansi+"/upload",
                cache: false,
                contentType: false,
                processData: false,
                complete: function(response) {
	...
	function deleteFile(file) {
            ...
            $.ajax({
                ...
                method: "POST",
				cache: false,
                url: ServeUrl+"/pages/"+Instansi+"/unupload",
                complete: function(response) {
					if(response.status == 200){
