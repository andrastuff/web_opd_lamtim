					...
					<div class="card-body">
									...
									<div class="tab-content">
										...
										<div class="tab-pane" id="left-icon-tab2">
											<form role="form" id="form-instansi">
											<input type="hidden" name="oldfoto">

											<table class="detail-view table table-striped table-condensed " id="yw0">
											<tbody>
											<tr class="odd"><th>Nama Kepala</th><td><input name="nama_kepala" type="text" class="form-control" value=""></br></td></tr>

											<tr class="odd"><th>Kabupaten</br></th><td><input name="kabupaten" type="text" class="form-control"></br></td></tr>

											<tr class="even"><th>Provinsi</th><td><input name="provinsi" type="text" class="form-control" value=""></br></td></tr>

											<tr class="even"><th>Foto Kepala</th><td><input name="userfile" type="file" class="form-control" value=""></td></tr>

											<tr class="even"><th> </th><td id="foto"></td></tr>
											</tbody>
											</table></br>
											<div class="content-divider">
												<span class="pt-10 pb-10"> </span>
...
<script>
	...
	function loadSetting(){
		$.ajax({
					data: "",
					url: ServeUrl+"/setting/"+Instansi+"/list",
          method: 'GET',
	...
	function loadInstansiSetting(){
	 
		$.ajax({
					data: "",
					url: ServeUrl+"/setting/"+Instansi+"/instansi_setting",
          method: 'GET',
          complete: function(response){ 
					if(response.status == 200){
							 
							 $("input[name=nama_kepala]").val(response.responseJSON.data.nama_kepala);
							 $("input[name=kabupaten]").val(response.responseJSON.data.kabupaten);
							 $("input[name=provinsi]").val(response.responseJSON.data.provinsi);
							 $("input[name=oldfoto]").val(response.responseJSON.data.foto_kepala);
							 $("#foto").html('<img class="img-thumbnail" width="80" src="'+response.responseJSON.data.foto_kepala+'" alt="">');
	...
	$("#form-settings").submit(function(event) {
		event.preventDefault();
		 swal({
				...
				$.ajax({
							data: data,
							url: ServeUrl+"/setting/"+Instansi+"/update/"+Instansi,
							processData: false,
							contentType: false,
							cache: false,
							timeout: 600000,
    ...
    $("#form-instansi").submit(function(event) {
		event.preventDefault();
		 swal({
                title: 'Are you sure?',
                text: "You won't be able to revert this!",
				...
				$.ajax({
							data: data,
							url: ServeUrl+"/setting/"+Instansi+"/update_setting_instansi",
							processData: false,
							contentType: false,
							cache: false,
							timeout: 600000,
	...
	$("#form-maps").submit(function(event) {
		event.preventDefault();
		 swal({
				...
				$.ajax({
							data: data,
							url: ServeUrl+"/setting/"+Instansi+"/updatemaps",
							type: 'POST',
							complete: function(response){                
							  if(response.status == 201){
