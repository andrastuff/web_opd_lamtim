			...
			<span id="alert"></span>
				...
				<div class="card border-2 border-dark">
					...
					<div class="loader text-center mt-5 mb-5"></div>
						<table class="table datatable table-bordered table-striped table-hover table-lg" id="datatable-tags">
...
<script>
	...
	$('#datatable-tags tbody').on( 'click', '#edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
		window.location.href = BaseUrl+"/admin/tags/update/"+data['id'];
    } );

	$('#datatable-tags tbody').on( 'click', '#delete', function () {
        var data = table.row( $(this).parents('tr') ).data();
		remove(data['id']); 
    } );

	function add(){
		window.location.href = BaseUrl+"/admin/tags/add/"; 
	}
	...
	function remove(id){
	          swal({
				...
				$.ajax({
							data: "",
							url: ServeUrl+"/tags/"+Instansi+"/delete/"+id,
							crossDomain: false,
							method: 'GET',
							complete: function(response){                
