					...
					<div class="col-md-8">
						...
						<div class="card" data-animation="fadeIn">
							...
							<div class="card-body d-sm-flex align-items-sm-center justify-content-sm-between flex-sm-wrap">
								...
								<div class="d-flex align-items-center mb-3 mb-sm-0">
									<a href="{{ url('admin/user/add') }}" class="btn bg-teal"><i class="icon-user-plus mr-2"></i> Create User</a>
								</div>
							</div>
...
<script>	
	function loadData(){
		$.ajax({
			data: "",
			url: ServeUrl+"/user/"+Instansi+"/list",
			crossDomain: true,
			method: 'GET',
			complete: function(response){                
				if(response.status == 200){
					...
					var tbody = '';
					var no = 0;
					tbody +='<li class="media font-weight-semibold border-0 py-1 h5 text-success">'+response.responseJSON.data.instansi.instansi+'</li>';
						$.each(response.responseJSON.data.data, function(x,y){
							...
							tbody +='<li><a href="#" class="media" data-toggle="collapse" data-target="#as'+y.id+'">'+
								'<div class="mr-2"><i class="icon-user"></i></div><div class="media-body"><div class="media-title font-weight-semibold">'+y.name+'</div><span class="text-muted">'+y.hak_akses+'</span>'+
								...
								'</a><div class="collapse" id="as'+y.id+'"><div class="card-body bg-light border-top border-bottom"><ul class="list list-unstyled mb-0">'+
										...
										'<li><i class="icon-mail5 mr-2"></i> <a href="#">'+y.email+'</a></li><li><a href="'+BaseUrl+'/admin/user/view/'+y.id+'" class="btn btn-link">Open Detail</a></li></ul></div></div></li>';
						});
					$('#users').html(tbody);  
