									...
									<form action="#" id="form-user-create">
										...
										<div class="form-group">
											<div class="row">
												...
												<div class="col-md-6">
													<label>Phone</label>
													<input name="phone" type="text" value="" class="form-control" maxlength="13">
												</div>
											</div>
										</div>
										...
										<div class="form-group">
											<div class="row">
													...
													<div class="col-md-6">
															...
															<input name="password" type="password" value="" class="form-control" autocomplete="new-password">
...
/*	$( "input[name=email]" ).change(function() {
    ...
    $("#form-user-create").submit(function(event) {
		...
		swal({
				...
				if($.isNumeric(id)){
					var path = ServeUrl+"/user/"+Instansi+"/update/"+id;
				}
	...
	function loadView(){	
		$.ajax({
			...
			url: ServeUrl+"/user",
					...
					complete: function(response){ 				
					if(response.status == 200){
                                $('input[name=id]').val(response.responseJSON.id);
								$('input[name=name]').val(response.responseJSON.name);
								$('input[name=username]').val(response.responseJSON.username);
								$('input[name=email]').val(response.responseJSON.email);
								$('input[name=address]').val(response.responseJSON.address);
								$('input[name=hak_akses]').val(response.responseJSON.hak_akses);
								$('input[name=phone]').val(response.responseJSON.phone);
								$('input[name=old_pw]').val(response.responseJSON.password);
						}else if(response.status == 401){
							e('info','401 server conection error');
						}
					},
